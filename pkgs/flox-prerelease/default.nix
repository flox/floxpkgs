...
Adding an attribute to a canonical re-export such as
    ...
    { inputs }:
      inputs.flox-main.packages.flox // {fromCatalog = self.evalCatalog.stable.flox; }

requires the evaluation of `inputs.flox-main.packages.flox` to an attrset.
...
*/
{
  ...
  inherit (inputs.flox-main.packages.flox) outputs out man outPath outputName drvPath name system;

  # re-export metadata
  meta = inputs.flox-main.packages.flox.meta;
