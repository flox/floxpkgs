...
in
  {lib ? nixpkgs.lib}: let
    ...
    */
    ...
    in
      builtins.mapAttrs apply {
        ...
        git4 = {
          expr =
            "git+https://github.com/flox/flox.git"
    ...
    in
      builtins.mapAttrs apply {
        ...
        tarball1 = {
          ...
          expected =
            "tarball+http://registry.npmjs.org"
            + "/lodash/-/lodash-4.17.21.tgz";
        };
        tarball2 = {
          expr = {
        ...
        git7 = {
          ...
          expected =
            "git+ssh://git@github.com/flox/flox.git"
            + "?ref=refs/heads/main";
        };
        git8 = {
          expr = {
        ...
        git10 = {
          ...
          expected =
            "git://git@github.com/flox/flox.git"
            + "?ref=main&rev=a3a3dda3bacf61e8a39258a0ed9c924eeca8e293";
        };
    ...
    */
    ...
    genTests = parent: let
      genTest = name: test: {
        ...
        value = {
          ...
          expected =
            if
              (builtins.isAttrs test.expected)
    ...
    in
      tests: builtins.attrValues (builtins.mapAttrs genTest tests);
    ...
    tests = let
      sets = builtins.mapAttrs genTests {
        inherit
          ...
          flakeRefAttrsToStrTests
          ;
      };
    in
      builtins.listToAttrs (builtins.concatLists (builtins.attrValues sets));
  ...
  in {
    ...
    checkDrv = {
      ...
      derivation {
        ...
        # in an awkward way.
        # In `impure' mode we use `builtins.currentTime' and in `pure' mode we
        # hash this file and the file we're testing.
        # In pure mode we may still attempt substitution but it's a relatively
        # good method of forcing rebuilds.
        forceDirty = let
        ...
        args = [
          ...
          ''
            if [[ -n "$pass" ]]; then
              ...
              echo 'failures:'             >&2;
              echo "$fails"                >&2;
              # We exit failure so the resulting store path is only preserved
              # if the user explicitly sets `--keep-failed'.
              echo 'FAIL'   > "$out";
              echo "$fails" > "$out";
              exit 1;
            fi
