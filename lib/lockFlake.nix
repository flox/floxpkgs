...
let
  ...
  paramStrToAttrs = str: let
    ...
    rsl =
      if builtins.elem str ["?" ""]
      then {}
      else builtins.listToAttrs (builtins.foldl' proc [] sp);
    pretty = toPretty str;
  in
  ...
  identifyURIType = ref: let
    ...
    data = let
      ...
      s =
        if f == null
        then builtins.elemAt m 4
        else f;
    in
      if builtins.elem s ["http" "https"]
      then null
      else s;
    ...
    dt =
      if data == null
      then null
      else builtins.getAttr data dataSchemeToType;
    ...
    rsl =
      ...
      else if test "${reFId}(/.*)?" ref
      then "indirect"
      else throw "`${pretty}' is not a valid URL.";
  in
    assert builtins.isString ref;
  ...
  in
    ref: let
      ...
      forT =
        ...
        then forGit
        else if builtins.elem type ["github" "sourcehut" "gitlab"]
        then forGHLike
        else forUrl;
      rsl =
        if (m == null) && (type != "indirect")
        then throw ("Invalid URI: " + ref)
        else base // forT;
      pretty = toPretty ref;
      msg = "Converting flakeref `" + pretty + "' to attributes.";
    in
      assert builtins.isString ref;
        builtins.addErrorContext msg rsl;
  ...
  flakeRefAttrsToStr = attrs: let
    ...
    in
      builtins.replaceStrings ["tarball+tarball" "git+git" "hg+hg"]
      ["tarball" "git" "hg"]
      wdata;
    # Add `refOrRev'
    rr = attrs.rev or attrs.ref or null;
    wr =
      if (rr == null) || (attrs.type == "git")
      then base
      else base + "/" + rr;
  ...
  lockFlake = let
    mkRef = r:
      assert (builtins.isString r) || (builtins.isAttrs r); {
        attrs =
          if builtins.isString r
          then flakeRefStrToAttrs r
          else r;
        string =
          if builtins.isString r
          then r
          else flakeRefAttrsToStr r;
      };
  in
    ref: let
      ...
      keeps =
        if builtins.elem originalRef.attrs.type ["tarball" "file"]
        then {
          rev = true;
          narHash = true;
        }
      ...
      lockedAttrs =
        (removeAttrs originalRef.attrs ["ref"])
        // (builtins.intersectAttrs keeps flake.sourceInfo);
      pretty = toPretty ref;
    in
      assert (builtins.isString ref) || (builtins.isAttrs ref);
